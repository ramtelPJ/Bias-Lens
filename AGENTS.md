...
8. On approval, re-read the approved prompt files in /prompt and implement strictly. Implement only after user approval.
...
- `design-taste-frontend`: overall visual direction for pages/redesigns â€” layout, typography, color, avoiding generic/templated UI
- `impeccable`: focused UI polish and review â€” hierarchy, spacing, accessibility, states, copy, on specific components or screens
- `apple-design`: motion and interaction feel â€” transitions, gestures, spring/easing choices, depth and materials
- `ui-ux-pro-max`: design system and styling tokens â€” consistent Tailwind/shadcn tokens, spacing scale, component styling patterns

For frontend UI work, read the skill(s) relevant to the task before implementing (e.g. `design-taste-frontend` for a new page's overall look, `impeccable` for polishing an existing component, `apple-design` for adding motion, `ui-ux-pro-max` for design tokens/system consistency). These skills only inform visual/UX decisions on top of the existing Tailwind/shadcn stack (section 6) â€” they must not introduce new frontend frameworks, component libraries, or architecture that conflicts with section 5's layering.

Do not invent new skills beyond the ones listed above.
...
| `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`                                        | Supabase publishable key (current key format, replaces legacy anon key)                 | client + server |
